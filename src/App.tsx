...
// Проверить, пятница ли сейчас по Москве
const isFridayInMoscow = (): boolean => {
  const moscowTime = getMoscowTime();
  return moscowTime.getDay() === 5; // 5 = пятница
};

// Получить ближайшую субботу (для записи в пятницу)
const getThisSaturday = (): Date => {
  const moscowTime = getMoscowTime();
  const dayOfWeek = moscowTime.getDay();
  
  // Если пятница (5), суббота завтра
  if (dayOfWeek === 5) {
    const saturday = new Date(moscowTime);
    saturday.setDate(moscowTime.getDate() + 1);
    return saturday;
  }
  
  // Если суббота или воскресенье, показываем следующую субботу
  const daysUntilSaturday = (6 - dayOfWeek + 7) % 7 || 7;
  const nextSaturday = new Date(moscowTime);
  nextSaturday.setDate(moscowTime.getDate() + daysUntilSaturday);
  return nextSaturday;
};
...
function TelegramMiniApp() {
  ...
  // Проверяем, пятница ли сейчас по Москве
  const isFriday = isFridayInMoscow();
  
  const nextSaturday = getThisSaturday();
  ...
  if (submitted && submittedBooking) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-orange-100 to-orange-200 flex items-center justify-center p-4">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-100 to-orange-200">
      {/* Header */}
      <div className="bg-gradient-to-r from-stone-800 to-stone-700 text-white p-6 rounded-b-3xl shadow-lg">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-stone-300 text-sm">Творим вместе</p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        ...
        {!loading && !isFriday && (
          <div className="bg-white rounded-2xl shadow-md p-5">
            <div className="text-center py-6">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">Запись открывается в пятницу</h3>
              <p className="text-gray-600 mb-4">
                Записаться на мастер-класс можно только в пятницу на ближайшую субботу.
              </p>
              <div className="bg-amber-50 rounded-xl p-4 text-sm text-amber-800">
                <p>Приходите в пятницу, чтобы записаться на субботний мастер-класс!</p>
              </div>
            </div>
          </div>
        )}
        ...
        {!loading && isFriday && (
          <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-md p-5 space-y-5">
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              <span>✍️</span> Форма записи на {formatDate(nextSaturday)}
            </h2>
        ...
        {/* Информация когда не пятница */}
        {!isFriday && (
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 border border-blue-200">
            <p className="text-blue-800 text-sm text-center">
              💡 <strong>Совет:</strong> Добавьте бота в избранное, чтобы не забыть записаться в пятницу!
            </p>
          </div>
        )}
