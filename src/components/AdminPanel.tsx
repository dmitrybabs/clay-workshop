...
import { Booking, PRICE_PER_HOUR } from '../types/booking';
...
export function AdminPanel() {
  ...
  const getFullName = (booking: Booking): string => {
  ...
  const totalRevenue = filteredBookings.reduce((acc, b) => acc + (b.totalPrice || b.hours * PRICE_PER_HOUR), 0);
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 mt-2">Студия керамики «Майолика»</p>
          </div>
  ...
  // Показываем панель рассылки
  if (showBroadcast) {
    return <BroadcastPanel onBack={() => setShowBroadcast(false)} />;
  }
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Студия керамики «Майолика»</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <p className="text-2xl font-bold text-blue-600">
              {filteredBookings.filter(b => b.gender === 'male').length}
            </p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm">
            <p className="text-gray-500 text-sm">Девочек</p>
            <p className="text-2xl font-bold text-pink-600">
              {filteredBookings.filter(b => b.gender === 'female').length}
            </p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm">
            <p className="text-gray-500 text-sm">Всего часов</p>
            <p className="text-2xl font-bold text-amber-600">
              {filteredBookings.reduce((acc, b) => acc + b.hours, 0)}
            </p>
          </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {Object.entries(getBookingsByTimeSlot(selectedDate)).map(([time, booking]) => (
                ...
                >
                  ...
                  {booking && (
                    <p className="text-sm text-amber-700 truncate">{getFullName(booking)}</p>
                  )}
                </div>
              ))}
            </div>
              ...
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Имя Фамилия</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filteredBookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">
                        <a href={`tel:${booking.parentPhone}`} className="hover:text-amber-600">
                          {booking.parentPhone || '—'}
                        </a>
                      </td>
                      <td className="px-4 py-3 text-sm font-medium text-green-600">
                        {(booking.totalPrice || booking.hours * PRICE_PER_HOUR).toLocaleString('ru-RU')} ₽
                      </td>
                      <td className="px-4 py-3">
                        <button
